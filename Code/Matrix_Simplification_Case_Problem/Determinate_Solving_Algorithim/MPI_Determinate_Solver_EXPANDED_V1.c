...
int determinate(int r, int* ptr, int rank, int size, int NumberOfTwoByTwos, int y) {
	...
	/* DEFINE THIS EVERY TIME */
	int* inner_detArray = (int*)calloc(3, sizeof(int));
	int* signArray = (int*)calloc(3, sizeof(int));
	int* basicArray = (int*)calloc(3, sizeof(int));
				...
				int* q = (int*)calloc(((r - 1) * (r - 1)), sizeof(int));
					...
					for (int i = 0; i < r; i++) {
						...
						/* MPI_Bcast must be called by all processes in order to complete */
							MPI_Bcast(&inner_detArray[0], 1, MPI_INT, 0, MPI_COMM_WORLD);
							MPI_Bcast(&basicArray[0], 1, MPI_INT, 0, MPI_COMM_WORLD);
							MPI_Bcast(&signArray[0], 1, MPI_INT, 0, MPI_COMM_WORLD);
